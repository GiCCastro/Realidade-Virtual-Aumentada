...
import type { EstadoDeRecurso } from '../src/devices/recursos';
import {
  classificarAparelho,
  type GrausDeLiberdade,
  type ClasseDeAparelho,
} from '../src/devices/grau';
import type {
  CapacidadesXR,
  ResultadoRecurso,
} from './sonda';
...
function tabelaDeRecursos(
  recursos: readonly ResultadoRecurso[],
): HTMLTableElement {
  ...
  for (const recurso of recursos) {
    ...
    linha.appendChild(celula(recurso.nome));
    linha.appendChild(celula(recurso.paraQueServe));
    linha.appendChild(celula(rotuloDoEstado(recurso.estado)));
  }
...
export function montarSonda(
  ...
  raiz.appendChild(paragrafo(resultado.mensagem));
  ...
  raiz.appendChild(subtitulo('Modos suportados'));

  raiz.appendChild(
    paragrafo(
      resultado.modosSuportados.length > 0
        ? resultado.modosSuportados.join(', ')
        : 'Nenhum modo imersivo suportado.',
    ),
  );

  raiz.appendChild(subtitulo('Classificação do aparelho'));

  raiz.appendChild(
    paragrafo(`Classe: ${resultado.classe}`),
  );

  raiz.appendChild(
    paragrafo(
      `Graus de liberdade: ${resultado.grausDeLiberdade}`,
    ),
  );

  raiz.appendChild(
    paragrafo(
      resultado.espacosConcedidos.length > 0
        ? `Espaços concedidos: ${resultado.espacosConcedidos.join(', ')}`
        : 'Nenhum espaço de referência foi concedido.',
    ),
  );

  raiz.appendChild(subtitulo('Recursos consultados'));

  raiz.appendChild(
    tabelaDeRecursos(resultado.recursos),
  );
}
